...
pub const TWO_ADIC_PRIMITVE_ROOT_OF_UNITY: U256 = UnsignedInteger::from_hex_unchecked(
...
pub fn get_roots_of_unity(order: u64) -> Vec<FE> {
    ...
    let mut roots_new: Vec<FE> = vec![FE::zero(); chunk];

    for i in 1..(roots.len() / chunk) {
        // let roots_new: Vec<FE> = roots[(i-1)*chunk..i*chunk].par_iter().map(|x| x * root_pow).collect();
        // let start = std::time::Instant::now();  
        roots_new.par_iter_mut().enumerate().for_each(|(j, x)| {
            *x = roots[(i-1)*chunk + j] * root_pow;
        });
        // println!("Roots of unity computation for chunk {} took {:?}", i, start.elapsed());
...
pub fn get_coset(base: FE, order: u64) -> Vec<FE> {
    let num_threads = current_num_threads();
    let chunk = 1 << 12;

    let mut coset = vec![FE::zero(); order as usize];
    coset[0] = FE::one();
    for i in 1..chunk {
        coset[i] = coset[i-1] * base;
    }
    let coset_pow = coset[chunk-1] * base;
    let mut coset_new: Vec<FE> = vec![FE::zero(); chunk];
    for i in 1..(coset.len() / chunk) {
        coset_new.par_iter_mut().enumerate().for_each(|(j, x)| {
            *x = coset[(i-1)*chunk + j] * coset_pow;
        });
        coset[i*chunk..(i+1)*chunk].copy_from_slice(&coset_new);
    }
...
pub fn parallel_fft_coset(coefs: &[FE], roots_of_unity: &[FE], coset: &[FE], log_size: usize, log_blowup_factor: usize) -> Vec<FE> {
    let new_coefs = coefs.iter().zip(coset.iter()).map(|(c, cos)| c * cos).collect::<Vec<_>>();
    parallel_fft(&new_coefs, roots_of_unity, log_size, log_blowup_factor)
}
...
pub fn hash_leaves(unhashed_leaves: &[[FE; 2]]) -> Vec<[u8; 32]> {
    ...
    (0..num_threads).into_par_iter().for_each(|i| {
        ...
        let local_hashed_chunk: Vec<[u8; 32]> = chunk_unhashed.iter().map(|unhashed| {
            ...
            to_be_hashed.extend_from_slice(&unhashed[0].as_bytes());
            to_be_hashed.extend_from_slice(&unhashed[1].as_bytes());
