                ...
                new_level_iter.par_chunks_mut(chunk_size)
                    ...
                    .for_each(|(new_level, children)| {
                        new_level.iter_mut().zip(children.chunks_exact(2)).for_each(|(new_parent, children)| {
                            ...
                            to_be_hashed.extend_from_slice(&children[0]);
                            to_be_hashed.extend_from_slice(&children[1]);
                            let hash = blake3::hash(&to_be_hashed);
                            new_parent.copy_from_slice(hash.as_bytes());
                        });
                    });
            } else {
...
pub fn folding_RS(
    ...
    let mut current_codeword = vec![FE::zero(); codelength];
    current_codeword[..codelength].copy_from_slice(codeword);
    let mut new_codeword = vec![FE::zero(); codelength];
    let mut current_coset_inv = vec![FE::zero(); codelength];
    current_coset_inv[..codelength].copy_from_slice(coset_inv);
    let mut new_coset_inv = vec![FE::zero(); codelength];
    let mut current_layer: FriLayer;
    let mut fri_layer_list = Vec::with_capacity(log_degree);
    let mut current_size = codelength;
    let FE_two_inv = (FE::one() + FE::one()).inv().unwrap();
    let mut zetas = Vec::<FE>::new();
    for layer in 0..log_degree {
        // Create Merkle tree for the current layer
        let to_commit: Vec<[FE; 2]> = current_codeword[..current_size].par_chunks_exact(2).map(|chunk| [chunk[0].clone(), chunk[1].clone()]).collect();
        let merkle_tree = MerkleTree::build(&to_commit);
        current_layer = FriLayer::new(
        ...
        current_size /= 2;
        new_coset_inv[..current_size].par_iter_mut().enumerate().for_each(|(i, new_coset_element)| {
            *new_coset_element = current_coset_inv[2*i] * current_coset_inv[2*i];
        });
        new_codeword[..current_size].par_iter_mut().enumerate().for_each(|(i, new_eval)| {
            let even = &current_codeword[2*i];
            let odd = &current_codeword[2*i + 1];
            *new_eval = FE_two_inv * ((even + odd) + current_coset_inv[2*i] * zeta * (even - odd));
        });
        current_codeword[..current_size].copy_from_slice(&new_codeword[..current_size]);
        current_coset_inv[..current_size].copy_from_slice(&new_coset_inv[..current_size]);
    }

    (
        current_codeword[0].clone(),
        fri_layer_list,
        zetas,
    )
}
...
) -> FriLayer
{
    ...
    let mut evaluation = parallel_fft_coset(&poly.coefficients, roots_of_unity, coset, log_poly_degree, log_blowup_factor);
...
{
    let FE_two_inv = FE::from(2).inv().unwrap();
    ...
    if !fri_layers.is_empty() {
        ...
        query_list = iotas
            ...
            .map(|iota_s| {
                ...
                for layer in fri_layers {
                    // symmetric element
                    println!("Index: {}, eval: {:?}, eval_sym: {:?}", index, layer.evaluation[index], layer.evaluation[index ^ 1]);
                    let evaluation_sym = layer.evaluation[index ^ 1].clone();
                    let auth_path_sym = layer.merkle_tree.get_proof_by_pos(index >> 1);
                    layers_evaluations_sym.push(evaluation_sym);
                    layers_auth_paths_sym.push(auth_path_sym);
...
pub fn verify_RS_folding(
    ...
    let evaluation_point_inv = coset_inv[iota].clone();
...
) -> bool
{
    ...
    let mut to_be_hashed = Vec::<u8>::new();
    to_be_hashed.extend_from_slice(&evaluations[0].as_bytes());
    to_be_hashed.extend_from_slice(&evaluations[1].as_bytes());
    let hash = blake3::hash(&to_be_hashed);
    let mut hashed_value = [0u8; 32];
    hashed_value.copy_from_slice(hash.as_bytes());

    let mut index = iota >> 1;
    for sibling_node in auth_path_sym.merkle_path.iter() {
        to_be_hashed.clear();
        if index % 2 == 0 {
            to_be_hashed.extend_from_slice(&hashed_value);
            to_be_hashed.extend_from_slice(sibling_node);
        } else {
            to_be_hashed.extend_from_slice(sibling_node);
            to_be_hashed.extend_from_slice(&hashed_value);
        }
