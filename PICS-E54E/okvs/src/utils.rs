...
pub fn hash_to_index(x: FE, r1: &[u8], range: usize) -> usize {
    ...
    to_be_hashed.extend_from_slice(&x.to_bytes_le());
    to_be_hashed.extend_from_slice(r1);
    let hash = blake3::hash(&to_be_hashed);
    let mut res = [0u8; 8];
    res.copy_from_slice(&hash.as_bytes()[..8]);
...
pub fn hash_to_band(x: FE, r2: &[u8]) -> U256 {
    let mut to_be_hashed = Vec::<u8>::new();
    to_be_hashed.extend_from_slice(&x.to_bytes_le());
    to_be_hashed.extend_from_slice(r2);
    let hash = blake3::hash(&to_be_hashed);
    let mut res = [0u8; 32];
    res.copy_from_slice(&hash.as_bytes()[..32]);
    U256::from_little_endian(&res)
}

pub fn hash_to_bin(x: FE, r3: &[u8], num_bins: usize) -> usize {
    let mut to_be_hashed = Vec::<u8>::new();
    to_be_hashed.extend_from_slice(&x.to_bytes_le());
    to_be_hashed.extend_from_slice(r3);
    let hash = blake3::hash(&to_be_hashed);
    let mut res = [0u8; 8];
    res.copy_from_slice(&hash.as_bytes()[..8]);
    usize::from_le_bytes(res) % num_bins
}
        ...
        for j in (i + 1)..rows {
            ...
            if bands_FE[j][pivot[i] - start_pos[j]] != FE::zero() {
                ...
                for k in 0..(band_width - first_nonzero[i]) {
                    bands_FE[j][k + offset] = bands_FE[j][k + offset] - lead * bands_FE_i[k + first_nonzero[i]];
                }
