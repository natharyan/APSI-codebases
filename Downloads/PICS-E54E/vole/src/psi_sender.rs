...
pub struct OprfSender {
    ...
    X_new_fri_layers: Vec<FriLayer>,
    X_new_last_value: FE,
    X_new_merkle_roots: Vec<[u8; 32]>,
    X_new_evaluations: Vec<FE>,
    X_new_decommit: Vec<FriDecommitment<F>>,
...
impl OprfSender {
    pub fn new<IO: CommunicationChannel>(
        ...
        let r2 = r[1];
        let r3 = r[2];
        let okvs = BinRbOkvs::new(2*n, 65, num_okvs_bins, &r1, &r2, &r3);
        ...
        OprfSender {
            ...
            b: vec![FE::zero(); fixed_points_num],
            K: vec![FE::zero(); fixed_points_num],
            delta: delta,
            okvs: okvs,
            w: FE::zero(),
            ...
            X_new_fri_layers: vec![],
            X_new_last_value: FE::zero(),
            X_new_merkle_roots: vec![],
            X_new_evaluations: vec![],
            X_new_decommit: vec![],
    ...
    pub fn commit_phase<IO: CommunicationChannel>(&mut self, io: &mut IO, values: &[FE], comm: &mut u64) {
        ...
        if self.sender_commit || self.receiver_commit {
        ...
        if self.sender_commit {
            ...
            (self.X_new_last_value, self.X_new_fri_layers) = commit_poly(
                &self.X_new_poly, 
            ...
            self.X_new_merkle_roots = self.X_new_fri_layers
                .iter()
                .map(|fri_layer| fri_layer.merkle_tree.root)
                .collect(); 

            self.sender_transcript.append_bytes(&self.X_new_merkle_roots[0]);  
            self.sender_transcript.append_bytes(&self.X_new_last_value.to_bytes_le());

            // Generate iotas for degree test of X_new
            let iotas: Vec<usize> = (0..NUM_QUERIES).map(|_| {
                let iota_bytes = self.sender_transcript.sample(8);
                ...
                self.sender_transcript.append_bytes(&iota_bytes);
                iota as usize
            }).collect();

            // Open degree test commitment of X_new
            self.X_new_evaluations = iotas
                .iter()
                .map(|&iota| self.X_new_fri_layers[0].evaluation[iota])
                .collect();

            self.X_new_decommit = query_phase(&self.X_new_fri_layers, &iotas);
            self.X_new_commit = self.X_new_fri_layers[0].clone();
        }

        if self.receiver_commit {
            self.P_new_merkle_root = io.receive_block::<32>().expect("Failed to receive merkle root of P")[0];
            self.receiver_transcript.append_bytes(&self.P_new_merkle_root);
        }

        if self.sender_commit {
            let X_new_merkle_root = self.X_new_merkle_roots[0];
            io.send_block::<32>(&[X_new_merkle_root])
                .expect("Failed to send merkle root of X_new");
        }
    }

    pub fn send<IO: CommunicationChannel>(&mut self, io: &mut IO, values: &[FE], comm: &mut u64) {
        // Send proof of degree of P
        if self.sender_commit {
            *comm += io.send_stark252(&[self.X_new_last_value]).expect("Cannot send last value of Q");
            println!("X_new_last_value = {:?}", self.X_new_last_value);   
            *comm += io.send_block::<32>(&self.X_new_merkle_roots).expect("Failed to send merkle root of Q");
            println!("X_new_merkle_roots = {:?}", self.X_new_merkle_roots);
            *comm += io.send_stark252(&self.X_new_evaluations).expect("Cannot send evaluations of Q");
            send_decommit(io, &self.X_new_decommit, comm);
        }
        ...
        *comm += io.send_stark252(&[hash_buf]).expect("Failed to send H(ws) to the receiver");
        ...
        // Running Vole
        // c = b + a * delta
        let mut z = vec![FE::zero(); self.fixed_points_num];
        self.vole_sender.extend(io, &mut self.b, &mut z, self.fixed_points_num, comm); 
        ...
        *comm += io.send_stark252(&[ws]).expect("Failed to send ws to the receiver");
        ...
        if self.sender_commit {
            self.sender_transcript.append_bytes(&hash_buf.to_bytes_le());
            self.sender_transcript.append_bytes(&wr.to_bytes_le());
            self.sender_transcript.append_bytes(&self.w.to_bytes_le());
        }
        if self.receiver_commit {
    ...
    pub fn prepare_output_consistency(&mut self, values: &[FE]) {
        ...
        if self.sender_commit {
            ...
            self.sender_transcript.append_bytes(&self.S_new_commit.merkle_tree.root);
            let t = self.sender_transcript.sample_field_element();
            ...
            self.sender_transcript.append_bytes(&self.T_new_last_value.to_bytes_le());
            self.sender_transcript.append_bytes(&self.T_new_merkle_roots[0]);
        }
        ...
        self.outputs_byte.par_iter_mut().enumerate().for_each(|(i, outputs_i)| {
            ...
            to_be_hashed.extend_from_slice(&values[i].as_bytes());
            to_be_hashed.extend_from_slice(&o[i].as_bytes());
            let hash = blake3::hash(&to_be_hashed);
            outputs_i.copy_from_slice(hash.as_bytes());
        });
    }
    ...
    pub fn receive_vole_consistency<IO: CommunicationChannel>(&mut self, io: &mut IO, A: &[FE], comm: &mut u64) {
        if self.receiver_commit {
            ...
            self.receiver_transcript.append_bytes(&Q_last_value.to_bytes_le());
            self.receiver_transcript.append_bytes(&Q_merkle_root);
