...
impl OprfReceiver {
    pub fn new<IO: CommunicationChannel>(
        ...
        let r2 = r[1];
        let r3 = r[2];
        let okvs = BinRbOkvs::new(n*2, 65, num_okvs_bins, &r1, &r2, &r3);
        io.send_block::<16>(&r);
    ...
    pub fn commit_phase<IO: CommunicationChannel>(&mut self, io: &mut IO, values: &[FE], comm: &mut u64) {
        ...
        if self.receiver_commit {
            ...
            *comm += io.send_block::<32>(&[self.P_new_commit.merkle_tree.root]).expect("Failed to send merkle root of P_new");
            println!("P_new merkle root = {:?}", self.P_new_commit.merkle_tree.root);
            self.receiver_transcript.append_bytes(&self.P_new_commit.merkle_tree.root);
        }
        
        if self.sender_commit {
            self.X_new_merkle_root = io.receive_block::<32>().expect("Failed to receive merkle root of X_new")[0];
            self.sender_transcript.append_bytes(&self.X_new_merkle_root);
        }
    ...
    pub fn receive<IO: CommunicationChannel>(&mut self, io: &mut IO, values: &[FE], comm: &mut u64) {
        if self.sender_commit {
            let X_new_last_value = io.receive_stark252().expect("Failed to receive X_new last value")[0];
            println!("X_new_last_value = {:?}", X_new_last_value);   
            let X_new_merkle_roots = io.receive_block::<32>().expect("Failed to receive merkle roots of X_new");
            println!("X_new_merkle_roots = {:?}", X_new_merkle_roots);
            let X_new_evaluations = io.receive_stark252().expect("Cannot receive evaluations of X_new");
            let X_new_decommit = receive_decommit(io, comm);
            assert_eq!(self.X_new_merkle_root, X_new_merkle_roots[0], "The merkle root of X_new does not match the expected value");

            self.sender_transcript.append_bytes(&X_new_last_value.to_bytes_le());
            ...
            iotas.par_iter().enumerate().for_each(|(i, &iota)| {
                let result = verify_fri_query(
                    ...
                    iota,
                    X_new_evaluations[i],
                    X_new_decommit[i].layers_evaluations_sym[0],
                    &self.roots_of_unity,
                );
                if result == false {
                    panic!("Receiver lied about X_new");
                }
            });
            println!("X passed the degree test!");
        ...
        *comm += io.send_stark252(&[wr]).expect("Failed to send wr to the sender");
        ...
        if self.sender_commit {
            self.sender_transcript.append_bytes(&hws.to_bytes_le());
            self.sender_transcript.append_bytes(&wr.to_bytes_le());
            self.sender_transcript.append_bytes(&w.to_bytes_le());
        }
        ...
        *comm += io.send_stark252(&A).expect("Failed to send A to the sender");
        ...
        for i in 0..self.n {
            ...
            receiver_outputs[i].copy_from_slice(hash.as_bytes());
        }
    ...
    pub fn receive_output_consistency<IO: CommunicationChannel>(&mut self, io: &mut IO, comm: &mut u64) {
        if self.sender_commit {
            ...
            self.sender_transcript.append_bytes(&self.S_new_merkle_root);
            ...
            self.sender_transcript.append_bytes(&T_new_last_value.to_bytes_le());
            self.sender_transcript.append_bytes(&T_new_merkle_roots[0]);
    ...
    pub fn send_vole_consistency<IO: CommunicationChannel>(&mut self, io: &mut IO, c: &[FE], comm: &mut u64) {
        if self.receiver_commit {
            ...
            *comm += io.send_block::<32>(&Q_merkle_roots).expect("Failed to send merkle root of Q");
            self.receiver_transcript.append_bytes(&self.Q_last_value.to_bytes_le());
            self.receiver_transcript.append_bytes(&Q_merkle_roots[0]);  
        ...
        if self.sender_commit {
            ...
            for _ in 0..self.log_fixed_points_num - 1 {
                ...
                new_sender_outputs_hash
                    ...
                    .for_each(|(j, hash)| {
                        let mut to_be_hashed = Vec::<u8>::new();
                        to_be_hashed.extend_from_slice(&sender_outputs_hash[2 * j]);
                        to_be_hashed.extend_from_slice(&sender_outputs_hash[2 * j + 1]);
            ...
            for i in 0..verify_path.len() {
                let mut to_be_hashed = Vec::<u8>::new();
                to_be_hashed.extend_from_slice(&ref_root);
                to_be_hashed.extend_from_slice(&verify_path[i]);
                let hash = blake3::hash(&to_be_hashed);
                ref_root.copy_from_slice(hash.as_bytes());
            }
